...
let surfBreaksTableReady = false;

async function ensureSurfBreaksTable() {
  if (surfBreaksTableReady) return;
  await getPool().execute(`
    CREATE TABLE IF NOT EXISTS surf_breaks (
      id          INT          NOT NULL AUTO_INCREMENT,
      name        VARCHAR(255) NOT NULL,
      tagline     VARCHAR(255) NOT NULL DEFAULT '',
      breaks_json LONGTEXT,
  ...
  `);
  surfBreaksTableReady = true;
}

function parseSurfBreakRow(row) {
  let breaks = [];
  let photos = [];
  try { breaks = JSON.parse(row.breaksJson || '[]'); } catch { breaks = []; }
  try { photos = JSON.parse(row.photosJson || '[]'); } catch { photos = []; }
  return { ...row, breaks, photos, breaksJson: undefined, photosJson: undefined };
}

export async function listSurfBreaks() {
  await ensureSurfBreaksTable();
  const [rows] = await getPool().execute(
    `SELECT id, name, tagline, breaks_json AS breaksJson, photos_json AS photosJson,
            sort_order AS sortOrder, enabled, created_at AS createdAt, updated_at AS updatedAt
     FROM surf_breaks
     ORDER BY sort_order ASC, id ASC`
  );
  return rows.map(parseSurfBreakRow);
}

export async function listEnabledSurfBreaks() {
  await ensureSurfBreaksTable();
  const [rows] = await getPool().execute(
    `SELECT id, name, tagline, breaks_json AS breaksJson, photos_json AS photosJson,
            sort_order AS sortOrder, enabled, created_at AS createdAt, updated_at AS updatedAt
...
export async function createSurfBreak({ name, tagline, breaks, photos, sortOrder, enabled }) {
  await ensureSurfBreaksTable();
  const [result] = await getPool().execute(
    `INSERT INTO surf_breaks (name, tagline, breaks_json, photos_json, sort_order, enabled)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [
      String(name || '').trim(),
      String(tagline || '').trim(),
  ...
  const [rows] = await getPool().execute(
    `SELECT id, name, tagline, breaks_json AS breaksJson, photos_json AS photosJson,
            sort_order AS sortOrder, enabled, created_at AS createdAt, updated_at AS updatedAt
     FROM surf_breaks WHERE id = ? LIMIT 1`,
    [result.insertId]
  );
  return parseSurfBreakRow(rows[0]);
}

export async function updateSurfBreak(id, { name, tagline, breaks, photos, sortOrder, enabled }) {
  await ensureSurfBreaksTable();
  const [result] = await getPool().execute(
    `UPDATE surf_breaks
     SET name = ?, tagline = ?, breaks_json = ?, photos_json = ?,
...
export async function deleteSurfBreak(id) {
  await ensureSurfBreaksTable();
  const [result] = await getPool().execute('DELETE FROM surf_breaks WHERE id = ?', [Number(id)]);
  return result.affectedRows > 0;
}

// ── Surf Retreat Inquiries ─────────────────────────────────────────────────

let inquiriesTableReady = false;

async function ensureInquiriesTable() {
  if (inquiriesTableReady) return;
  await getPool().execute(`
    CREATE TABLE IF NOT EXISTS surf_retreat_inquiries (
      id INT NOT NULL AUTO_INCREMENT,
      ...
      arrival_date DATE,
      departure_date DATE,
      surf_level VARCHAR(100),
      num_guests VARCHAR(50),
  ...
  `);
  inquiriesTableReady = true;
}

export async function saveRetreatInquiry({
  ...
  const [result] = await getPool().execute(
    `INSERT INTO surf_retreat_inquiries
     (full_name, country, email, whatsapp, arrival_date, departure_date,
      surf_level, num_guests, duration, lesson_type, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      ...
      arrivalDate || null,
      departureDate || null,
      String(surfLevel || '').trim(),
      String(numGuests || '').trim(),
      String(duration || '').trim(),
      String(lessonType || '').trim(),
      String(notes || '').trim(),
    ]
  );
