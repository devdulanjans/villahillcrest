...
import { defaultTermsContent } from '../../lib/terms-defaults';
...
export default function AdminTermsAndConditionsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          setError(data.message || 'Failed to load terms content');
          return;
        }

        const item = data.item || defaultTermsContent;
        const nextForm = {
          sectionTitle: item.sectionTitle || defaultTermsContent.sectionTitle,
          bodyHtml: item.bodyHtml || defaultTermsContent.bodyHtml,
        };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.message || 'Failed to save terms content');
        return;
      }

      setMessage('Terms & Conditions saved successfully');
    } catch {
      setError('Failed to save terms content');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <>
      <Head>
        <title>Admin Terms & Conditions | Villa Hillcrest</title>
      </Head>

      <div className={styles.page}>
        <AdminSidebar activeLabel="Terms & Conditions" />

        <main className={styles.main}>
          <header className={styles.topBar}>
            <h2>Terms & Conditions Manager</h2>
          </header>

          <section className={styles.panel}>
            <h3>{loading ? 'Loading...' : 'Update Terms & Conditions'}</h3>
            ...
            <form className={styles.form} onSubmit={handleSubmit}>
              ...
              <div className={styles.formActions}>
                <button type="submit" disabled={busy || loading}>{busy ? 'Saving...' : 'Save Terms & Conditions'}</button>
              </div>
            </form>
          </section>
